...
reference_points = point_clouds(:,:,5); %pose5_points  !!!change to first pose in future Version
% points = point_clouds(:,:,28);
points = point_clouds;
example_measurement = 21;
...
% Creating plot
scatter3(ax, point_clouds(:,1,5), point_clouds(:,2,5), point_clouds(:,3,5), 'g', 'x');
...
f = @(x) find_sph_coeff(points(:,:,21:24), reference_points, gradUnwarp, x);    %numeric function with vector input
% x0 = rand(1, nvar) * 100 - 50;   %starting point
% f = @(x) x^2+2;
% x0=1;
options = optimset('TolFun', 1e-12,'PlotFcns',@optimplotfval);
