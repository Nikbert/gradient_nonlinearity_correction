...
point_clouds2 = pose1_points;
for i = 2:number_of_calib_files2
    number = int2str(i);
    pose_points = get_points(append(path, 'calibration_', number ,'.calib'));
    point_clouds2 = cat(3,point_clouds2,pose_points);
end
...
%%
p1column = point_clouds(:,:,1:8);
p2column = point_clouds(:,:,17:24);
p3column = point_clouds(:,:,25:31);
p4column = point_clouds(:,:,42:48);
p5column = point_clouds(:,:,49:55);
p6column = point_clouds(:,:,67:72);
p7column = point_clouds(:,:,73:77);
p8column = point_clouds(:,:,95:96);
% p9column = point_clouds(:,:,97:103);
% p10column = point_clouds(:,:,116:120);
% p11column = point_clouds(:,:,121:122);
measured_points1 = cat(3,p1column,p2column,p3column,p4column,p5column,p6column,p7column,p8column);
measured_points = measured_points1;
...
x
save('x0_thresh0_0001_mri_ref_small_set.mat','x');
%%
[sum_vecnorm_difference_pose, translated_rotated_points] = find_sph_coeff(measured_points(:,:,example_measurement), reference_points, gradUnwarp, x);
% [sum_vecnorm_difference_pose, translated_rotated_points] = find_sph_coeff_multi(measured_points, reference_points, gradUnwarp, x);
...
%     xxlr = reshape(gradUnwarp.mri_to_lab(xxdr), size(xxdr));
%     reference_points = xxlr'/1000;
    reference_points = reference_points_d;
