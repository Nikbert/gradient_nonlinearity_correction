...
methods
    ...
    function XD = lab_to_mri(obj, XL)
        % transform coordinates from lab frame to image frame
    ...
    function XL = mri_to_lab(obj, XD)
        ...
        XL=zeros(size(XD));
        for i=1:size(XD,2)
            XL(:,i)=fminsearch(@(x) vecnorm(obj.lab_to_mri(x)-XD(:,i)), XD(:,i));
        end
    end
...
% void GradUnwarp::read_siemens_coeff(const char *gradfilename)
    function read_siemens_coeff(obj, gradfilename)
